    ...
    def write_weights(self, fname):
        with open(fname, 'w+') as f:
            ...
            for begin, end in self.weight_fun.get_intervals():
                ...
                # Normalize to [0,100]
                max_weight = max((weight_func(e) for e in self.net.getEdges()), default=1)
                ...
                f.write(f'    <interval id="{int(begin)}_{int(end)}" begin="{begin}" end="{end}" totalWeight="{total:.2f}">\n')
...
class LoadedProps:
    def __init__(self, fname):
        ...
        for interval in sumolib.xml.parse_fast(fname, 'interval', ['id', 'begin', 'end']):
            attr_dict = dict(interval)
            begin = float(attr_dict['begin'])
            end = float(attr_dict['end'])

            weights = {}
            for edge in interval.findall('edge'):
                edge_id = edge.attrib['id']
                value = float(edge.attrib['value'])
                weights[edge_id] = value

            self.interval_weights[(begin, end)] = weights
