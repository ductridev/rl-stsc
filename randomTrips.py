...
def get_options(args=None):
    ...
    op.add_argument(
        ...
        type=float,
        default=0.0,
        help="weight edge probability by speed^'FLOAT' (default 0)",
    )
    op.add_argument(
        "--fringe-speed-exponent",
        category="weights",
        dest="fringe_speed_exponent",
        metavar="FLOAT",
    ...
    op.add_argument(
        ...
        default=False,
        help="Determine fringe edges based on junction attribute 'fringe'",
    )
    op.add_argument(
        "--vclass",
        "--edge-permission",
    ...
    # processing
    op.add_argument("-s", "--seed", default=42, type=int, help="random seed")
    op.add_argument(
        "--random",
        action="store_true",
        default=False,
    ...
    # flow
    op.add_argument(
        "-b", "--begin", category="flow", default=0, type=op.time, help="begin time"
    )
    op.add_argument(
        "-e",
        "--end",
    ...
    if options.vclass and not is_vehicle_class(options.vclass):
        raise ValueError(
            "The string '%s' doesn't correspond to a legit vehicle class."
            % options.vclass
        )
    ...
    if (
        options.period is None
        and options.insertionRate is None
        and options.insertionDensity is None
    ):
        options.period = [1.0]
    ...
    if options.insertionDensity:
        ...
        options.insertionRate = [
            density * (length / 1000.0) for density in options.insertionDensity
        ]

    if options.insertionRate:
        options.period = [
            3600.0 / rate if rate != 0.0 else 0.0 for rate in options.insertionRate
        ]
        ...
        if options.binomial:
            for p in options.period:
                if p != 0.0 and 1.0 / p / options.binomial >= 1:
                    print(
                        "Warning: Option --binomial %s is too low for insertion period %s."
                        % (options.binomial, p)
                        + " Insertions will not be randomized.",
                        file=sys.stderr,
                    )
    ...
    if options.vehicle_class:
        ...
        if "type=" in options.tripattrs:
            raise ValueError(
                "Trip-attribute 'type' cannot be used together with option --vehicle-class."
            )

    if options.randomDepartPos:
        if "departPos" in options.tripattrs:
            raise ValueError(
                "Trip-attribute 'departPos' cannot be used together with option --random-departpos."
            )

    if options.randomArrivalPos:
        if "arrivalPos" in options.tripattrs:
            raise ValueError(
                "Trip-attribute 'arrivalPos' cannot be used together with option --random-arrivalpos."
            )

    if options.weightsprefix:
        weight_files = [
            options.weightsprefix + s for s in (SOURCE_SUFFIX, DEST_SUFFIX, VIA_SUFFIX)
        ]
        if not any([os.path.isfile(w) for w in weight_files]):
            raise ValueError(
                "None of the weight files '%s' exists." % "', '".join(weight_files)
            )
...
def loadStops(options):
    ...
    if options.additional is None:
        print(
            "Error: Option %s requires option --additional-files for loading infrastructure elements"
            % ("--from-stops" if options.fromStops else "--to-stops"),
            file=sys.stderr,
        )
        sys.exit(1)
    stopTypes = []
    ...
    if options.fromStops:
        ...
        if available == 0:
            print(
                "No stops of type%s '%s' were found in additional-files %s"
                % (
...
class RandomEdgeGenerator:
    ...
    def write_weights(self, fname):
        with open(fname, "w+") as f:
            ...
            # Loop over all intervals from the weight provider
            for begin, end in self.weight_fun.get_intervals():
                weight_func = self.weight_fun.get_weight_func_for_interval(begin, end)

                # Normalize to [0,100]
                max_weight = max(
                    (weight_func(e) for e in self.net.getEdges()), default=1
                )
                normalizer = 100.0 / max_weight

                weights = [
                    (weight_func(e) * normalizer, e.getID())
                ...
                f.write(
                    f'    <interval id="{int(begin)}_{int(end)}" begin="{begin}" end="{end}" totalWeight="{total:.2f}">\n'
                )
                for w, eid in weights:
...
class RandomTripGenerator:
    ...
    def __init__(
        self, source_generator, sink_generator, via_generator, intermediate, pedestrians
    ):
    ...
    def get_trip(
        ...
        for min_dist in [min_distance, min_dist_fringe]:
            ...
            for _ in range(maxtries):
                ...
                intermediate = [
                    self.via_generator.get() for __ in range(self.intermediate)
                ]
                sink_edge = self.sink_generator.get()
                is_fringe2fringe = (
                    source_edge.is_fringe()
                    and sink_edge.is_fringe()
                ...
                coords = (
                    [source_edge.getFromNode().getCoord()]
...
def get_prob_fun(options, fringe_bonus, fringe_forbidden, max_length):
    ...
    def edge_probability(edge):
        bonus_connections = (
            None if fringe_bonus is None else getattr(edge, fringe_bonus)
        )
        forbidden_connections = (
            None if fringe_forbidden is None else getattr(edge, fringe_forbidden)
        )
        ...
        if (
            fringe_forbidden is not None
            and edge.is_fringe(forbidden_connections)
            and not options.pedestrians
            and (
                options.allow_fringe_min_length is None
                or edge.getLength() < options.allow_fringe_min_length
            )
        ):
            return 0  # the wrong kind of fringe
        if (
            fringe_bonus is not None
            and options.viaEdgeTypes is not None
            and not edge.is_fringe(
                bonus_connections, checkJunctions=options.fringeJunctions
            )
            and edge.getType() in options.viaEdgeTypes
        ):
        ...
        if options.fringe_factor != 1.0 and fringe_bonus is not None:
            isFringe = edge.getSpeed() > options.fringe_threshold and edge.is_fringe(
                bonus_connections, checkJunctions=options.fringeJunctions
            )
...
def parse_custom_structure(data: str) -> Any:
    data = data.strip()
    ...
    # Match main structure: attributes and child_dict content
    match = re.match(r'<\[(.*?)\],child_dict={(.*)}>', data, re.DOTALL)
    if not match:
        return None

    attr_str, child_dict_raw = match.groups()

    # Parse attributes
    attrs = re.findall(r"\('([^']+)', '([^']+)'\)", attr_str)
    attr_dict = dict(attrs)

    # Parse child_dict manually (no regex-based object parsing!)
    children_dict = {}
    # We assume format like: 'edge': [<[...],child_dict={}>, ...]
    key_value_matches = re.finditer(r"'([^']+)'\s*:\s*\[(.*)\]$", child_dict_raw.strip(), re.DOTALL)
    for kv_match in key_value_matches:
        key, items_raw = kv_match.groups()

        items = split_items(items_raw.strip())
        children_dict[key] = [parse_custom_structure(item) for item in items]
...
def split_items(s: str) -> list[str]:
    """
    Splits a string containing multiple <[...],child_dict={}> blocks.
    Works regardless of spacing or newlines between items.
    """
    pattern = r"<\[(.*?)\],\s*child_dict=\{\}>"  # non-greedy match of each item
    matches = re.finditer(pattern, s, re.DOTALL)
    return [f"<[{m.group(1)}],child_dict={{}}>" for m in matches]
...
class LoadedProps:
    def __init__(self, fname):
        ...
        # Use sumolib.xml.parse to parse elements properly
        for interval in sumolib.xml.parse(fname, "interval"):
            interval = parse_custom_structure(str(interval))

            begin = float(interval["attributes"]["begin"])
            end = float(interval["attributes"]["end"])
            self.intervals.append((begin, end))

            edge_weights = {}
            for edge in interval["children"].get("edge", []):
                edge_id = edge["attributes"]["id"]
                weight = float(edge["attributes"]["value"])
                edge_weights[edge_id] = weight

            self.interval_weights[(begin, end)] = edge_weights
    ...
    def get_weight_func_for_interval(self, begin, end):
        edge_weights = self.interval_weights.get((begin, end), {})

        def func(edge):
            return edge_weights.get(edge.getID(), 0.0)
    ...
    try:
        ...
        if options.weightsprefix:
            if os.path.isfile(options.weightsprefix + SOURCE_SUFFIX):
                props = LoadedProps(options.weightsprefix + SOURCE_SUFFIX)
                begin, end = props.get_intervals()[0]
                source_generator = RandomEdgeGenerator(
                    net, props.get_weight_func_for_interval(begin, end)
                )
        ...
        print(
            "Error: no valid edges for generating source or destination. Try using option --allow-fringe",
            file=sys.stderr,
        )
        return None

    try:
        via_generator = RandomEdgeGenerator(net, get_prob_fun(options, None, None, 1))
...
def main(options):
    ...
    if options.min_distance > options.net.getBBoxDiameter() * (
        options.intermediate + 1
    ):
        options.intermediate = (
            int(math.ceil(options.min_distance / options.net.getBBoxDiameter())) - 1
        )
        print(
            (
                "Warning: Using %s intermediate waypoints to achieve a minimum trip length of %s in a network "
                "with diameter %.2f."
            )
            % (
                options.intermediate,
                options.min_distance,
                options.net.getBBoxDiameter(),
            ),
            file=sys.stderr,
        )
    ...
    time_delta = (parseTime(options.end) - parseTime(options.begin)) / len(
        options.period
    )
    times = [
        parseTime(options.begin) + i * time_delta
        for i in range(len(options.period) + 1)
    ]
    ...
    def generate_attributes(
        ...
        if options.toStops:
            attrTo = ' %s="%s"' % random.choice(
                options.edgeToStops[destination.getID()]
            )
        via = ""
        if intermediate:
    ...
    def generate_one_plan(
        combined_attrs, attrFrom, attrTo, arrivalPos, intermediate, options
    ):
        element = "walk"
    ...
    def generate_one_flow(
        label, combined_attrs, departureTime, arrivalTime, period, options, timeIdx
    ):
        ...
        if options.binomial:
            for j in range(options.binomial):
                fouttrips.write(
                    (
                        '    <flow id="%s#%s" begin="%s" end="%s" probability="%.2f"%s/>\n'
                    )
                    % (
                        label,
                        j,
                        departureTime,
                        arrivalTime,
                        1.0 / period / options.binomial,
                        combined_attrs,
                    )
                )
        elif options.poisson:
            fouttrips.write(
                ('    <flow id="%s" begin="%s" end="%s" period="exp(%s)"%s/>\n')
                % (
                    label,
                    departureTime,
                    arrivalTime,
                    1 / (period * options.flows),
                    combined_attrs,
                )
            )
        else:
            fouttrips.write(
                ('    <flow id="%s" begin="%s" end="%s" period="%s"%s/>\n')
                % (
                    label,
                    departureTime,
                    arrivalTime,
                    intIfPossible(period * options.flows),
                    combined_attrs,
                )
            )

    def generate_one_personflow(
        label,
        combined_attrs,
        attrFrom,
        attrTo,
        ...
        if options.binomial:
            for j in range(options.binomial):
                fouttrips.write(
                    (
                        '    <personFlow id="%s#%s" begin="%s" end="%s" probability="%.2f"%s>\n'
                    )
                    % (
                        label,
                        j,
            ...
            if options.poisson:
                fouttrips.write(
                    (
                        '    <personFlow id="%s" begin="%s" end="%s" period="exp(%s)"%s>\n'
                    )
                    % (
                        label,
                        departureTime,
                ...
                fouttrips.write(
                    ('    <personFlow id="%s" begin="%s" end="%s" period="%s"%s>\n')
                    % (
                        label,
                        departureTime,
                        arrivalTime,
    ...
    def generate_one(
        ...
        try:
            label, combined_attrs, attrFrom, attrTo, via, arrivalPos = (
                generate_attributes(
                    idx,
                    departureTime,
                    arrivalTime,
            ...
            if options.vtypeout:
                ...
                with open(options.vtypeout, "w") as fouttype:
                    sumolib.writeXMLHeader(
                        fouttype, "$Id$", "additional", options=options
                    )
                        ...
                        for time in departures:
                            ...
                            try:
                                origin, destination, intermediate = (
                                    generate_origin_destination(trip_generator, options)
                                )
                                idx = generate_one(
                                    idx,
                                    time,
        ...
        if router in options_to_forward:
            for option in options_to_forward[router]:
                ...
                if option[0] not in routerargs:
                    ...
                    raise ValueError(
                        "The argument '%s' has already been passed without the %s prefix."
                        % (option[0], router)
                    )
...
if __name__ == "__main__":
    try:
        if not main(get_options()):
            print(
                "Error: Trips couldn't be generated as requested. "
                "Try the --verbose option to output more details on the failure.",
                file=sys.stderr,
            )
            sys.exit(1)
    except ValueError as e:
        print("Error:", e.with_traceback(), file=sys.stderr)
        sys.exit(1)
