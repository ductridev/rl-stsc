...
def generate_and_save_random_intervals(
    sumo_cfg_file: str,
    total_duration=3600,
    min_interval=600,
    max_interval=1800,
    ...
    max_active_sides=2,
    edge_groups=None,
    filename_prefix="random_edge_priority",
):
    current_time = 0
    interval_id = 0
    ...
    while current_time < total_duration:
        ...
        # Create root element for each interval file
            ...
            if edges is not None:
                for edge_id in edges:
                    ET.SubElement(
                        interval,
                        "edge",
                        {"id": edge_id, "value": f"{weights[side]:.2f}"},
                    )

        # Save this interval to its own XML file
        file_name = f"{filename_prefix}_interval_{interval_id}.src.xml"
        full_path = os.path.join(directory, file_name)
        ET.ElementTree(root).write(full_path, encoding="utf-8", xml_declaration=True)
        print(f"Saved interval {interval_id} to {full_path}")
